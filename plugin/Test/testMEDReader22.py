...
def test():
    ...
    MyAssert(ds_bl.GetCellData().GetNumberOfArrays() in [3, 4] )#  3 for field, mesh and FamilyIdCell +1 for vtkGhostType
    ...
    for ds in [ds_bl,ds_br,ds_tl,ds_tr]:
        MyAssert(ds.GetPointData().GetNumberOfArrays() in [ 1, 2 ] )
